...
const char* select_pipeline_from_db = { "SELECT"
                                        "  mr_pipeline.pl_id,"
                                        ...
                                        "FROM"
                                        "  mr_pipeline"};
const char* select_script_from_db = {"SELECT"
                                     "  mr_script.scpt_id,"
                                     "  mr_script.scpt_name,"
                                     "  mr_script.host_id,"
...
const char* select_plexec_from_db = {
    "select * from mr_pl_exec"
};


const char* select_plen_from_db = {
"SELECT \
    mr_pln_exec.plne_id,\
    mr_pln_exec.pe_id,\
    mr_pln_exec.edge_id,\
...
DBHANDLE connect_db(const char *host, int port, const char *db, const char *user,
               const char *passwd) {
  //printf("connect_db ..... \n");
...
void get_pipeline(void *pipeline_ptr, MYSQL_ROW &mysql_row) {
    ...
    pl->pl_create_tm = atoll(mysql_row[4]);
    pl->pl_modify_tm = atoll(mysql_row[5]);
}
/*
...
void get_edge(void* edge_ptr, MYSQL_ROW& mysql_row) {
  ...
  edge->id      = atoll(mysql_row[0]);
  edge->pl_id        = atoll(mysql_row[1]);
  edge->src_id       = atoll(mysql_row[2]);
  edge->trg_id       = atoll(mysql_row[3]);
  ...
  edge->src_timeout  = atoi(mysql_row[8]);
  edge->trg_timeout  = atoi(mysql_row[9]);
  ...
  edge->create_tm    = atoll(mysql_row[15]);
  edge->modify_tm    = atoll(mysql_row[16]);
...
void get_pipeline_exec(void* ptr, MYSQL_ROW& row) {
    ...
    ple->id        = atoll(row[0]);
    ple->pl_id        = atoll(row[1]);
    ple->pe_stm       = atoll(row[2]);
    ple->pe_etm       = atoll(row[3]);
...
void get_plen_exec(void* ptr, MYSQL_ROW& row) {
    ...
    ne->id    = atoll(row[0]);
    ne->pe_id      = atoll(row[1]);
    ne->edge_id    = atoll(row[2]);
...
void get_host_status(void* ptr, MYSQL_ROW& row) {
    ...
    hs->id   = atoll(row[0]);
    hs->pe_id        = atoll(row[2]);
    hs->host_id      = atoll(row[3]);
    hs->host_status  = (HOST_STATUS_TYPE)(row[4][0] - '0');
    hs->updatetime   = atoll(row[4]);
}
